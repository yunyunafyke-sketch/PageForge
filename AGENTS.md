...
- `page-forge-boot`：启动类、配置文件以及其他模块的组装，不放业务实现。
- `page-forge-common`：统一响应、分页模型、通用请求、基础实体、全局异常和简单公共工具。
- `page-forge-security`：登录、JWT 生成与解析、登录状态及接口角色校验。
- `page-forge-system`：用户、角色、功能、菜单和数据字典等系统基础能力。
- `page-forge-business`：项目自身的业务功能；每个业务使用独立包，避免跨业务耦合。
- `sql`：数据库建表与初始化脚本。
